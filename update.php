...
include 'koneksi.php';
    ...
    if(empty($file_tmp)){
        ...
        nama='$nama',jeniskelamin='$jekel',kelas='$kelas',jurusan='$jurusan',photo='$namafile' WHERE npm='$npm'")or die(mysql_null());
    if($update){
            echo "<script>alert('DATA BERHASIL DIUPDATE');window.location='index1.php';</script>";
        }else{
            echo "<script>alert('UPDATE GAGAL');window.location='index1.php';</script>";
        }
        }else{
    ...
    if(move_uploaded_file($file_tmp, 'file/'.$namafile)){ 
        ...
        jeniskelamin='$jekel',kelas='$kelas',jurusan='$jurusan',photo='$namafile' 
        WHERE npm='$npm'")or die(mysql_null());
    if($update){
            ...
            window.location='edit_data.php?id=$npm';</script>";
        }
        }
        }
?>